...
object testmethod {
  def main(args: Array[String]): Unit = {
    ...
    var raw = sc.textFile("Data/yelp_reviews_clustering_large.txt")
    var feature = "W"
    var N = 5
    var iterations = 20
    ...
    //      sum
    //    }
    def comp_avg(member:Iterable[(Int, scala.collection.mutable.Map[Int, Double])], group_size: Int, dim: Int): scala.collection.mutable.Map[Int, Double]={
    ...
    def find_nearest(vect:scala.collection.mutable.Map[Int, Double], cent:List[(scala.collection.mutable.Map[Int, Double],Int)]): Int={
      var res = -1
    ...
    //      new_group_cent
    //    }
    def kmeans(vect: RDD[scala.collection.mutable.Map[Int, Double]], cent:List[(scala.collection.mutable.Map[Int, Double],Int)], dim:Int): List[(Int, scala.collection.mutable.Map[Int, Double])]={
      var vec_group = vect.map(vec=>{
...
//    def word2count(review:Array[String], word_map:Map[String, Int], Size:Int): Array[Int] ={
//      var count = new Array[Int](Size)
//      for(word <- review){
...
//    }
    ...
    //kmeans
    var cent = sc.parallelize(word_count.takeSample(withReplacement=false, num = N).zipWithIndex.map{case(v,i) => (v, i)}).collect().toList //(((001310),0),....)
//    var vec = word_count.collect().toList
//        word_count.foreach(println)
    for(i <- 0 until iterations){
      var new_cent = kmeans(word_count, cent, allword_size)
